namespace PlanetGeneration
{
    ...
    public partial class PlanetGenerator
    {
        ...
        SebPlateSample SampleSebPlateField(Vector3 spherePoint)
        {
            ...
            // Snapshot array references inside the lock so a concurrent
            // InvalidateSebPlateData() call on the main thread cannot null the
            // fields while we are iterating them on the background Task thread.
            Vector3[] centers;
            float[]   macroValues;
            lock (sebPlateDataLock)
            {
                centers     = sebPlateCenters;
                macroValues = sebPlateMacroValues;
            }

            if (centers == null || macroValues == null)
                return new SebPlateSample { primaryValue = 0.5f, secondaryValue = 0.5f, blendedValue = 0.5f };
            ...
            {
                float jx = FBM(centers[i] * macroF * 3.1f, 2, 0.5f, 2f, seed + i * 17);
                float jy = FBM(centers[i] * macroF * 3.1f + new Vector3(5.1f, 11.3f, 3.7f), 2, 0.5f, 2f, seed + i * 17 + 99);
                float jz = FBM(centers[i] * macroF * 3.1f + new Vector3(9.2f, 3.8f, 17.1f), 2, 0.5f, 2f, seed + i * 17 + 198);
                Vector3 jitteredCenter = (centers[i] + new Vector3(jx, jy, jz) * jitter).normalized;
                ...
                if (d < nearestDist)
                {
                    ...
                    nearestDist     = d;
                    primaryValue    = macroValues[i];
                }
                else if (d < secondDist)
                {
                    secondDist     = d;
                    secondaryValue = macroValues[i];
                }
            }

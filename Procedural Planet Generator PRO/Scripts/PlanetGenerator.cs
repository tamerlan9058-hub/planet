    ...
    public partial class PlanetGenerator : MonoBehaviour
    {
        ...
        void UpdateShaderParams()
        {
            if (planetMaterial != null)
            {
                ...
                if (planetMaterial.HasProperty("_RimStrength"))   planetMaterial.SetFloat("_RimStrength",   0.16f);
                if (planetMaterial.HasProperty("_RimPower"))      planetMaterial.SetFloat("_RimPower",      3.2f);
                if (planetMaterial.HasProperty("_SaturationBoost"))planetMaterial.SetFloat("_SaturationBoost", 1.35f);
                if (planetMaterial.HasProperty("_ShadowTint"))    planetMaterial.SetColor("_ShadowTint",    new Color(0.68f, 0.62f, 0.66f, 1f));
            }
            UpdateOceanMaterialParams();
        }

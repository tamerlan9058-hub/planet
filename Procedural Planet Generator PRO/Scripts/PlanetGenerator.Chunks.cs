namespace PlanetGeneration
{
    ...
    public partial class PlanetGenerator
    {
        ...
        float GetSurfaceSafetyDistance()    => Mathf.Max(radius * 0.24f, colliderDistance * 0.55f) + GetChunkWorldSpan() * 1.20f;
        float GetColliderSafetyDistance()   => Mathf.Max(colliderDistance * 0.82f, radius * 0.26f) + GetChunkWorldSpan() * 0.55f;
        ...
        IEnumerator GenerateChunkMesh(ChunkData chunk, int lod)
        {
            ...
            try
            {
                ...
                var mf = chunkObject.GetComponent<MeshFilter>()   ?? chunkObject.AddComponent<MeshFilter>();
                var mr = chunkObject.GetComponent<MeshRenderer>() ?? chunkObject.AddComponent<MeshRenderer>();
                mf.sharedMesh     = chunk.mesh;
                mr.sharedMaterial = planetMaterial;

                if (useColliders)
                {
                    var mc = chunkObject.GetComponent<MeshCollider>() ?? chunkObject.AddComponent<MeshCollider>();
        ...
        {
            if (terrainStyle == TerrainStyle.SebInspired && lod <= 0) return;
            ...
            {
                Vector3 va = vertices[idxA], vb = vertices[idxB];
                Vector3 da = va.normalized,  db = vb.normalized;
                int start  = vertices.Count;
                vertices.Add(va - da * skirtDepth);
                vertices.Add(vb - db * skirtDepth);
                uvs.Add(uvs[idxA]); uvs.Add(uvs[idxB]);
                colors.Add(colors[idxA]); colors.Add(colors[idxB]);
                normals.Add(normals[idxA]); normals.Add(normals[idxB]);

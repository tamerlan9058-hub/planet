namespace PlanetGeneration
{
    ...
    public class PlanetLODController : MonoBehaviour
    {
        ...
        internal void ApplyStreamingGrant(bool terrainGrant, bool prewarmGrant, int buildBudget)
        {
            ...
            float terrainVisibleCoverage = activePlanetApproach ? 0.16f : 0.48f;
            ...
            {
                float fadeStart = activePlanetApproach ? 0.01f : 0.08f;
                float fadeEnd = activePlanetApproach ? 0.14f : 0.42f;
                ...
                if (activePlanetApproach && localSurfaceCoverage > 0.01f)
                {
                    float distanceOpacity = Mathf.InverseLerp(
                        terrainGenerator.radius * 0.20f,
                        terrainGenerator.radius * 0.85f,
                        currentSurfaceDistance);
                    proxyOpacity = Mathf.Min(proxyOpacity, distanceOpacity);
                }
                showFar = false;

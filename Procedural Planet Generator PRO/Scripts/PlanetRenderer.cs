namespace PlanetGeneration
{
    ...
    public class PlanetRenderer : MonoBehaviour
    {
        ...
        public void SyncRuntimeProperties()
        {
            ...
            lod0Object.transform.localScale = Vector3.one * (proxyRadius * 0.9970f);
            lod1Object.transform.localScale = Vector3.one * (proxyRadius * 0.9945f);
        ...
        int GetProxyFaceResolution(ProxyDetailLevel detailLevel, bool mediumDetail)
        {
            switch (detailLevel)
            {
                case ProxyDetailLevel.Low:
                    return mediumDetail ? 8 : 5;
                case ProxyDetailLevel.Medium:
                    return mediumDetail ? 12 : 8;
                default:
                    return mediumDetail ? 18 : 12;
            }
        }
        ...
        Mesh GetOrBuildProxyMesh(ref Mesh targetMesh, ref int cachedResolution, string label, int faceResolution, bool forceRebuild)
        {
            ...
            {
                targetMesh = terrainGenerator.BuildProxyMesh(faceResolution, $"{label}_{proxyDetailLevel}_{faceResolution}");
                if (targetMesh != null)
                    return targetMesh;
            }

...
namespace PlanetGeneration
{
    ...
    public partial class SolarSystemGenerator
    {
        ...
        IEnumerator WarpCoroutine(PlanetEntry target)
        {
            ...
            {
                if (target.root != null)
                    planetCenter = target.root.transform.position;
                rayOrigin = planetCenter + up * rayStart;
                spawnPos  = planetCenter + up * (target.generator.radius + target.generator.heightMultiplier + 5f);
        ...
        IEnumerator PreparePlanetForArrival(PlanetEntry target, float timeout)
        {
            if (target.lodController != null)
                target.lodController.ForceTerrainStreaming(timeout);
